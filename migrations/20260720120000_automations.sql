create table if not exists automations (
  ...
  lifecycle text not null check (lifecycle in ('draft', 'live', 'disabled')),
...
create table if not exists automation_versions (
  ...
  state text not null check (state in ('draft', 'live', 'superseded')),
...
alter table automations add constraint automations_draft_version_fk foreign key (draft_version_id) references automation_versions(id) on delete set null;
alter table automations add constraint automations_live_version_fk foreign key (live_version_id) references automation_versions(id) on delete set null;
...
create table if not exists automation_schedules (
  ...
  state text not null check (state in ('active', 'running', 'disabled', 'blocked', 'completed')),
  next_run_at text,
...
create index if not exists automation_schedules_due_idx on automation_schedules (state, next_run_at) where state = 'active';
